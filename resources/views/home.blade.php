                    ...
                    <div class="carousel-item {{$item++ == 0 ? "active" : ""}} float-left w-full">
                        <a href="{{url('/article/'.$article->id)}}" class="w-full">
                            <div class="bg-[url({{asset($article->picture)}})] bg-cover bg-center bg-no-repeat">
                                <div class="h-[75vh] flex flex-col justify-center items-center z-2 bg-dark bg-opacity-80 w-full text-center">
                                    <h1 class="text-highlight text-[4vw] mb-2 font-bold text-center w-[75%]">{{$article->title}}</h1>
                                    ...
                                    <div class="flex flex-row justify-center items-center mt-10">
                                        @foreach ($article->categories as $category)
                                            <div href="{{url('/category/'.$category->id)}}" class="text-white font-bold px-4 py-2 mr-16 z-2 bg-white/[.2] rounded hover:bg-highlight transition-all ease-in duration-120">
                                                {{$category->name}}
                                            </div>
                                        @endforeach
                                    </div>
