            ...
            @auth
                @if (Auth::user()->id == $user->id)
                    <button data-bs-toggle="modal" data-bs-target="#editprofile"
                        class="font-bold text-white bg-highlight px-6 py-3 rounded-md hover:bg-dark hover:text-highlight transition-all ease-in duration-75 mb-5">Edit
                        Profile
                    </button>
                @endif
            @endauth
                        ...
                        <div class="modal-body relative p-8 font-montserrat">
                            ...
                            <form class="flex flex-col">
                                ...
                                <input type="text" id="name" name="name" placeholder="Name"
                                    class="mb-5 border-[1px] rounded p-3 border-dark/[0.5]" value="{{ $user->name }}">
                                <label class="font-medium mb-2">Email</label>
                                <input type="email" id="email" name="email" placeholder="Email"
                                    class="mb-5 border-[1px] rounded p-3 border-dark/[0.5]" value="{{ $user->email }}">
                                <label class="font-medium mb-2">Bio</label>
                                <textarea id="bio" name="bio" placeholder="Bio" class="mb-5 border-[1px] rounded p-3 border-dark/[0.5]"
